...
__kernel void render(__global volatile uint* buffer, __global uint* sprite, __global float2* pos, __global int* frame, int sprite_framesize, int frame_count)
{
...
__kernel void remove(__global uint* buffer, __global uint* backup, __global int2* lastPos, __global int* lastTarget, int sprite_framesize)
{
	...
	//atomic_xchg(buffer + lastPos[y].x + (lastPos[y].y + v) * MAP_WIDTH + u, backup[v * sprite_framesize + u + y * (sprite_framesize + 1) * (sprite_framesize + 1)]);
